...
function hapus_srtklr($id, $file)
{
    global $conn;
    $id_surat = $id;

    if (!unlink("../../fileSuratKeluar/" . $file)) {
        return false;
    }
...
function ubah_status($id, $status)
{
    ...
    if ($status == "anggota") {
        ...
        mysqli_query($conn, "UPDATE user SET `status` = 'anggota' WHERE id_user = $id");
    }
